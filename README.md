...
* **Access & Refresh JWT (HS256):**
  ...
  * **Защита от компрометации (Token Reuse Detection):** попытка использовать уже отозванный токен расценивается как атака и немедленно инвалидирует все активные сессии пользователя.
...
### 4. Алгоритмическая оптимизация: Merge Sort $O(n \log n)$
* Написана устойчивая (stable) утилита сортировки **Merge Sort** с гарантированной алгоритмической сложностью $O(n \log n)$ в худшем, среднем и лучшем случаях (в отличие от QuickSort $O(n^2)$).
* Поддержка естественной сортировки кириллицы (`Intl.Collator` / `localeCompare('ru', { numeric: true })`), ISO-дат и корректная обработка `null`/`undefined` значений.
