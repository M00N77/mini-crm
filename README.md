...
### 6. Оптимизация шрифтов и нулевой сдвиг макета (Zero-CLS & Font Metric Overrides)
...
* **Автоматический подгон метрик фоллбека (`adjustFontFallback: true`):**
  * Turbopack на этапе сборки анализирует геометрию глифов и генерирует синтетический `@font-face` для системного шрифта (`Arial` / `system-ui`) с точными CSS-модификаторами:
    * **`size-adjust`** — масштабирует пропорции глифов системного шрифта в процентах под *x-height* основного шрифта.
    * **`ascent-override`** — переопределяет высоту выносных элементов над базовой линией (*ascenders*).
    * **`descent-override`** — переопределяет глубину нижних выносных элементов под базовой линией (*descenders*).
    * **`line-gap-override`** — нормализует межстрочный интервал.
  * **Результат:** системный и кастомный шрифт занимают **пиксель-в-пиксель одинаковую площадь и высоту строки**, полностью устраняя сдвиг контента при свопе (**Cumulative Layout Shift / CLS = 0.000**).
